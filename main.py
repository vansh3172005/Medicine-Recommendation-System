...
symptoms_dict = {
    ...
    'pain_behind_the_eyes': 36, 'back_pain': 37,
    'constipation': 38, 'abdominal_pain': 39,
    ...
    'cramps': 65, 'bruising': 66, 'obesity': 67,
    ...
    'knee_pain': 78, 'hip_joint_pain': 79,
    'muscle_weakness': 80, 'stiff_neck': 81,
    'swelling_joints': 82, 'movement_stiffness': 83,
...
def predict_disease(symptoms):
    ...
    return [(disease_names[i], probs[i]) for i in top_idx]
...
def predict():
    ...
    if predictions is None:
        return render_template("index.html", message="Please enter at least 3 valid symptoms.")

    best_disease, best_prob = predictions[0]

    if best_prob < 0.4:
        return render_template("index.html", message="Symptoms are insufficient for accurate diagnosis.")

    desc, pre, med, die, wrkout = helper(best_disease)
